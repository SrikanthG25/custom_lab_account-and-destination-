...
class Accountupdation(generics.GenericAPIView):
    ...
    def post(self, request):
        try:
            ...
            Account.objects.create(**create_data)
    ...
    def get(self, request):
        try:
            data = request.GET
            account_id = request.account_id if hasattr('account_id' , request) else data.get("account_id")
    ...
    def put(self, request):
        try:
            data = request.data
            account_id = request.account_id if hasattr('account_id' , request) else data.get("account_id")
            app_secret_token = request.headers.get('token')
...
class Destinationview(generics.GenericAPIView):
    # permission_classes = (permissions.AllowAny)
    # authentication_classes = []
    def post(self, request):
        try:
    ...
    def get(self, request):
        try:
            data = request.GET
            account_id = request.account_id if hasattr('account_id' , request) else data.get("account_id")
            destination = Destination.objects.get(account_id=account_id)
            destination_data = {
                "id": destination.id,
    ...
    def put(self, request):
        try:
            ...
            account_id = request.account_id if hasattr('account_id' , request) else data.get("account_id")
            destination = Destination.objects.get(account_id=account_id)
            destination.url = data.get("url")
            destination.http_method = data.get("http_method")
            destination.headers = data.get("headers")
            destination.save()
